...
class StreamHandlerWithConsoleFormatterTest(unittest.TestCase, FormatterExtension):
    ...
    def test_pref_handling_01(self):
        ...
        self.assertRegex(lines[0], self._get_combined_regex(
            self._get_parsed_string(Levels.INFO.value, "Message 1", full_line=False, colored=self.colored, pref='^'), '\r',
    ...
    def test_pref_handling_04(self):
        ...
        self.assertRegex(lines[1], self._get_level_regex(Levels.INFO.value, "Message 2", full_line=True, colored=self.colored, pref='\t'))
    ...
    def test_pref_handling_05(self):
        ...
        self.assertEqual(2, len(lines))
        self.assertRegex(lines[0], self._get_level_regex(Levels.INFO.value, "Message 1", full_line=True, colored=self.colored))
        self.assertRegex(lines[1], self._get_level_regex(Levels.PLAIN.value, "Message 2", full_line=True, colored=self.colored, pref='\r'))
    ...
    def test_pref_handling_06(self):
        ...
        self.assertEqual(2, len(lines))
        self.assertRegex(lines[0], self._get_level_regex(Levels.INFO.value, "Message 1", full_line=True, colored=self.colored))
        self.assertRegex(lines[1], self._get_level_regex(Levels.INFO.value, "Message 2", full_line=True, colored=self.colored, pref='\r'))
    ...
    def test_pref_handling_07(self):
        ...
        self.assertEqual(2, len(lines))
        self.assertRegex(lines[0], self._get_level_regex(Levels.INFO.value, "Message 1", full_line=True, colored=self.colored))
        self.assertRegex(lines[1], self._get_level_regex(Levels.PLAIN.value, "Message 2", full_line=True, colored=self.colored, pref='\t'))
    ...
    def test_pref_handling_09(self):
        ...
        self.assertEqual(1, len(lines))
        self.assertRegex(lines[0], self._get_combined_regex(
            self._get_parsed_string(Levels.PLAIN.value, "Message 1", full_line=False, colored=self.colored, pref="^"), '\r',
            self._get_parsed_string(Levels.PLAIN.value, "Message 2", full_line=False, colored=self.colored, suffix="$")
        ))
